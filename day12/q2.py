...
# for q2 we set every 'a' (value is 1 in the map) as Start and store the minimun step to End.
min_steps=map.shape[0]*map.shape[1]
for index1,i in enumerate(map):
        ...
        if j==1:
            ...
            while open:
                ...
                if min_g_node(open).pos==End:
                    if min_g_node(open).g<min_steps:
                        min_steps=min_g_node(open).g
                    break
